...
namespace {
    ...
    auto CreateKeybindConfig() -> void {
        ...
        config << "# Keybind configuration values based on Virtual-Key Codes\n";
        config << "# https://learn.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes\n\n";
        config << "# Keys may also be in a human-readable format, e.g. F1, Space, Left, etc. Case insensitive.\n";
        config <<
            "# Keys may be directly copied from the \"Constant\" column in the Virtual-Key Codes table in the above documentation,\n";
        config << "# with or without the \"VK_\" prefix.\n";
        config << "# Some keys allow shorthand, e.g. N0 for Numpad0, * for Multiply, etc.\n";

        int maxKeyLength = 0;
        for (const auto& kb : keybinds) {
            const int len = static_cast<int>(std::strlen(kb.name));
            maxKeyLength = max(len, maxKeyLength);
        }
    ...
    auto GetVKeyCode(const std::string& str, unsigned char& out) -> bool {
        try {
            // Try hex value first
...
void ReadKeybindConfig() {
    ...
    while (std::getline(config, line)) {
        ...
        if (const size_t delimiter_pos = line.find('='); delimiter_pos != std::string::npos) {
            key = Trim(line.substr(0, delimiter_pos));
            value = Trim(line.substr(delimiter_pos + 1));
        }
        else {
            continue;
        }

        auto to_lower = [](const unsigned char c) { return std::tolower(c); };
        std::ranges::transform(value, value.begin(), to_lower);

        // Allow optional "VK_" prefix from documentation
        if (value.starts_with("vk_")) {
            value = value.substr(3);
        }

        unsigned char keycode;
        if (!GetVKeyCode(value, keycode)) {
            LogMessage("Invalid keycode {} for action {} in config, skipping", value, key);
            continue;
        }
        ...
        for (auto& kb : keybinds) {
            if (key == kb.name) {
                LogMessage("Mapped action {} to keycode {:#04x} from {}", key, keycode, value);
                kb.keycode = keycode;
                break;
            }
        }
